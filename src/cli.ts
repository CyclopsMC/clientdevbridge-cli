...
program
  ...
  .option('--port <port>', `localhost port for the bridge (default: the first free port from ${START_DEFAULTS.port})`)
  ...
  .option('--jdwp-port [port]', 'also open a JDWP debug port, required by hotswap; with no number, the first free one')
